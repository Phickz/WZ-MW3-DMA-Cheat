...
{
    int ingame = DMA::Read<int64_t>(DMA::BaseAddress + offsetsBNET::game_mode, sizeof(ingame));
...
{
    int playercount = DMA::Read<int>(DMA::BaseAddress + offsetsBNET::game_mode, sizeof(playercount));
...
{
    auto index = DMA::ReadPtr(client_info + offsetsBNET::local_index);
    std::cout << "Index Ptr: 0x" << std::hex << index << std::dec << std::endl;
    int read_return = DMA::Read<int>(index + offsetsBNET::local_index_pos, sizeof(read_return));
    return read_return;
}

inline auto is_player_valid(QWORD player_address) -> bool
{
    bool response = DMA::Read<bool>(player_address + offsetsBNET::player::valid, sizeof(response));
    return response;
}

inline auto team_id(QWORD player_address) -> int
{
    int team = DMA::Read<int>(player_address + offsetsBNET::player::team, sizeof(team));
    return team;
}

inline auto get_position(QWORD player_address) -> fvector
{
    auto local_position = DMA::ReadPtr(player_address + offsetsBNET::player::pos);
...
{
    auto player_camera = DMA::ReadPtr(DMA::BaseAddress + offsetsBNET::camera_base);
    if (!player_camera)
        return {};

    fvector received = DMA::Read<fvector>(player_camera + offsetsBNET::camera_pos, sizeof(received));
    return received;
}
auto get_angles() -> fvector2d
{
    auto camera_angle = DMA::ReadPtr(DMA::BaseAddress + offsetsBNET::camera_base);
    if (!camera_angle)
        return {};

    fvector2d received = DMA::Read<fvector2d>(camera_angle + offsetsBNET::camera_pos + 0xC, sizeof(received));
    return received;
}
...
{
    fvector information = DMA::Read<fvector>(Client_Information + offsetsBNET::bone::bone_base, sizeof(information));
    return information;
}

auto bone_pointer(uint64_t base, uint64_t index) -> uint64_t
{
    std::cout << "Address to BonePtr: 0x" << std::hex << base + (index * offsetsBNET::bone::size) + offsetsBNET::bone::offset << std::dec << std::endl;
    auto bone = DMA::ReadPtr(base + (index * offsetsBNET::bone::size) + offsetsBNET::bone::offset);
    return bone;
}
